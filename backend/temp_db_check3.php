...
echo "=== DATABASE INTEGRITY CHECKS (FIXED) ===\n\n";
...
$duplicateDeductions = DB::table('inventory_logs')
    ->select('inventory_item_id', DB::raw('COUNT(*) as deduction_count'))
...
// Check 11: Orders with payment proof but pending payment status
...
// Check 14: Orders with negative total amount
echo "14. Orders with negative total amount:\n";
$negativeAmount = DB::table('customer_orders')
    ->where('total_amount', '<', 0)
    ->get();
echo "Count: " . $negativeAmount->count() . "\n";
if ($negativeAmount->count() > 0) {
    echo "Sample records:\n";
    echo $negativeAmount->take(3)->toJson(JSON_PRETTY_PRINT) . "\n";
}
echo "\n";

// Check 15: Order items with negative quantity or price
echo "15. Order items with negative quantity or price:\n";
$negativeOrderItems = DB::table('customer_order_items')
    ->where(function($query) {
    ...
    ->get();
echo "Count: " . $negativeOrderItems->count() . "\n";
if ($negativeOrderItems->count() > 0) {
    echo "Sample records:\n";
    echo $negativeOrderItems->take(3)->toJson(JSON_PRETTY_PRINT) . "\n";
}
echo "\n";
